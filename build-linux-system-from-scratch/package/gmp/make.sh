...
(cd ${PKGBUILD_DIR} && rm -rf config.cache; eval "${TARGET_CONFIGURE_OPTS} CONFIG_SITE=/dev/null ./configure --host=${GNU_TARGET_NAME} --build=x86_64-pc-linux-gnu --prefix=/usr --enable-shared --disable-static --disable-nls")
step_end configure
